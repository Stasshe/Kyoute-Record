...
export default function GradesPage() {
  ...
  async function saveEdit(id: number) {
    ...
    try {
      ...
      // 新しいデータを挿入
      const { error: insertError } = await supabaseClient.from('grades').insert([
        {
          date: dateStr,
          subject: s,
          score: sc,
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        {user && (
          <form onSubmit={addGrade} className="flex items-end gap-3">
            ...
            <button type="submit" className="text-sm px-4 py-1 bg-black text-white rounded">
              追加
            </button>
          </form>
        )}
                      ...
                      <>
                        ...
                        <td className="px-3 py-2">
                          <select
                            ...
                            onChange={e =>
                              setEditingFields(p => ({ ...p, subject: e.target.value }))
                            }
                            className="text-sm px-2 py-1 border rounded"
                          >
                            {SUBJECTS.map(s => (
                        ...
                        <td className="px-3 py-2">
                          <input
                            ...
                            onChange={e =>
                              setEditingFields(p => ({ ...p, score: Number(e.target.value) }))
                            }
                            min="0"
                            max="100"
                            className="text-sm px-2 py-1 border rounded w-20"
